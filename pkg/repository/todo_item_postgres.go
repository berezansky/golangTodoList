...
func (r *TodoItemPostgres) Create(listId int, input consts.TodoItem) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]consts.TodoItem, error) {
	var items []consts.TodoItem
	query := fmt.Sprintf(
...
func (r *TodoItemPostgres) GetById(userId, itemId int) (consts.TodoItem, error) {
	var item consts.TodoItem
	query := fmt.Sprintf(
		`SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti 
			INNER JOIN %s li on li.item_id = ti.id 
				INNER JOIN %s ul on ul.list_id = li.list_id
...
func (r *TodoItemPostgres) Update(userId, itemId int, input consts.UpdateItemInput) error {
